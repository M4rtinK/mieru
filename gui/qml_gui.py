...
class QMLGUI(GUI):
  def __init__(self, mieru, type, size=(720, 1280)):
    #size = (1280, 768)
    size = (768, 1280)
    GUI.__init__(self, mieru)
    self.mieru = mieru
    ...
    self.app.setStartDragDistance(32)
    self.view = ModifiedQDeclarativeView(self)

    BBPYPATH = bbpy.BBPYPATH
    #BBPYPATH = os.path.dirname(os.path.dirname(__file__))
    fullPath = os.path.join(BBPYPATH, 'qt/imports')
    self.view.engine().addImportPath(fullPath)
    self.view.engine().addPluginPath(fullPath)

    #    # try OpenGl acceleration
    #    glw = QtOpenGL.QGLWidget()
    #    self.view.setViewport(glw)
    # NOTE: segfaults Python on BB10:
    # Process 23830736 (python3.2) terminated SIGSEGV code=1 fltno=11 ip=790cc7b4(/base/usr/lib/qt4/lib/libQtOpenGL.so.4.8.4@_ZN10QGLContext11makeCurrentEv+0x7) mapaddr=0004c7b4. ref=00000008
    # -> X11 dependency ?
    ...
    # commebted out due to BB10 breakage
...
class MangaPageImageProvider(QDeclarativeImageProvider):
  ...
  def requestImage(self, pathId, size, requestedSize):
    ...
    try:
      print("path id:%s" % pathId.encode('utf-8'))
      ...
      print('QML GUI: loading page failed')
      print(e)
...
class IconImageProvider(QDeclarativeImageProvider):
  ...
  def requestImage(self, iconFilename, size, requestedSize):
    ...
    print(iconFilename)
    try:
      f = open('icons/%s' % iconFilename, 'rb')
      img = QImage()
...
class ReadingState(QObject):
  ...
  def openManga(self, path):
    ...
    try:
      ...
      print('QML GUI: opeaning manga from path failed')
